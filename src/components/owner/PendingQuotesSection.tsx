...
export function PendingQuotesSection({ userId, onQuoteAction }: PendingQuotesSectionProps) {
  ...
  const fetchPendingQuotes = useCallback(async () => {
    try {
      ...
      const { data: workOrders, error: woError } = await supabase
        .from("work_orders")
        .select(`
          id, title, description, scheduled_date, estimated_arrival_time,
          is_emergency, business_id, wish_form_id,
          boat:boats(id, name)
        `)
        .in("boat_id", boatIds);
      ...
      const { data: quotes, error: qError } = await supabase
        .from("quotes")
        .select("id, work_order_id, base_price, materials_deposit, notes, created_at, business_id")
      ...
      const bizIds = [...new Set(quotes.map(q => q.business_id).filter(Boolean) as string[])];
  ...
  const handleAcceptQuote = async (quote: PendingQuote) => {
    ...
    try {
      ...
      const totalAmount = quote.base_price + (quote.materials_deposit || 0);
      ...
      // 3. Update wish to accepted via wish_form_id
      const wishFormId = quote.work_order?.wish_form_id;
      if (wishFormId) {
        await supabase
          .from("wish_forms")
          .update({ status: "accepted" })
        ...
        // 4. Decline all other pending quotes on sibling work orders for this wish
        const { data: siblingWOs } = await supabase
          .from("work_orders")
          .select("id")
          .eq("wish_form_id", wishFormId);

        if (siblingWOs && siblingWOs.length > 0) {
          const woIds = siblingWOs.map(wo => wo.id);
          await supabase
            .from("quotes")
  ...
  const handleDeclineQuote = async (quote: PendingQuote) => {
    ...
    try {
      ...
      toast({
        ...
        description: "The wish remains open for other providers to quote.",
      });
  ...
  if (loading || pendingQuotes.length === 0) return null;
    ...
    <section className="mt-8">
      ...
      <div className="space-y-4">
        {pendingQuotes.map((quote) => {
          const total = quote.base_price + (quote.materials_deposit || 0);
          return (
              ...
              <CardContent className="pt-4">
                ...
                <div className="bg-muted/50 rounded-lg p-4 mb-4">
                  <div className="space-y-2 text-sm">
                    ...
                    {quote.materials_deposit != null && quote.materials_deposit > 0 && (
                      <div className="flex justify-between">
