...
import { Plus, Package, Search, Pencil, Trash2, Wrench } from "lucide-react";
                    ...
                    <TableRow key={item.id} className={!item.is_active ? "opacity-50" : ""}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          {item.is_part && <Wrench className="h-3 w-3 text-primary" />}
                        </div>
                        {item.description && (
                      ...
                      <TableCell className="text-right font-medium">${item.retail_price.toFixed(2)}</TableCell>
                      <TableCell className="text-right">
                        <span className={cn(
                          "font-medium",
                          item.current_quantity <= item.reorder_point ? "text-destructive" : "text-foreground"
                        )}>
                          {item.current_quantity}
                        </span>
                        <span className="text-xs text-muted-foreground ml-1">/ {item.reorder_point} min</span>
                      </TableCell>
                      <TableCell>
